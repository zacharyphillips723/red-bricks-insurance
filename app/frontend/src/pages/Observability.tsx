  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [t, c] = await Promise.all([
        api.getObservabilityTraces().catch(() => ({ traces: [] })),
        api.getObservabilityCosts().catch(() => ({ costs: [] })),
      ]);
      setTraces(t.traces || []);
      setCosts(c.costs || []);
    } finally {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          MLflow traces and model usage for the care-intelligence multi-agent supervisor, captured in Unity Catalog OTel tables and system billing tables.
        </p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {(costs.length > 0 ? costs : fallbackCosts).map((c) => {
          const inTok = Number(c.total_input_tokens || 0);
          const outTok = Number(c.total_output_tokens || 0);
          const est = c.estimated_cost_usd != null ? Number(c.estimated_cost_usd) : 0;
      ...
      <div className="card">
        <div className="p-4 border-b border-gray-200 flex items-center justify-between">
          <h3 className="font-semibold text-gray-800">Recent Agent Traces</h3>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              ...
              {traces.length === 0 && <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-400">No traces yet. Ask a member agent a question to generate traces.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
