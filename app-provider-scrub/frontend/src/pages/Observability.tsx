...
import { Activity, Loader2, DollarSign, Zap, RefreshCw, ThumbsUp, ThumbsDown, MessageSquare } from "lucide-react";
import { api } from "@/lib/api";
import type { ObservabilityTrace, CostSummary, FeedbackRow } from "@/lib/api";
...
export function Observability() {
  ...
  const loadData = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [traceRes, costRes, fbRes] = await Promise.all([
        api.getTraces().catch(() => ({ traces: [] })),
        api.getCostSummary().catch(() => ({ costs: [] })),
        api.getRecentFeedback().catch(() => [] as FeedbackRow[]),
      ]);
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          MLflow traces and model usage for the denial-reasoning agent and scrub engine,
          captured in Unity Catalog OTel tables.
        </p>
      </div>
              ...
              {traces.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-gray-400">
                    No traces yet. Run a scrub to generate agent traces.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {/* Recent human feedback — MLflow assessments logged on the agent traces */}
      <div className="card">
        <div className="p-4 border-b border-gray-200 flex items-center gap-2">
          <MessageSquare className="w-4 h-4 text-databricks-red" />
          <h3 className="font-semibold text-databricks-dark">Recent Agent Feedback</h3>
          <span className="text-xs text-gray-400">Human ratings logged as MLflow assessments on the trace</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-600">Trace ID</th>
                <th className="px-4 py-3 text-right font-medium text-gray-600">When</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {feedback.map((f, i) => (
                <tr key={i}>
                  <td className="px-4 py-3">
                    {Number(f.value) >= 0 ? (
                      ...
                      <ThumbsDown className="w-4 h-4 text-databricks-red" />
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className="font-mono text-xs bg-gray-100 rounded px-2 py-0.5">{f.target}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-600 max-w-xs truncate" title={f.rationale || ""}>
                    {f.rationale || "—"}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-gray-500">
                    {f.trace_id ? String(f.trace_id).slice(0, 16) + "…" : "—"}
                  </td>
                  <td className="px-4 py-3 text-right text-gray-500">
              ...
              {feedback.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-gray-400">
                    No feedback yet. Rate a scrub result to log an assessment on its trace.
                  </td>
                </tr>
              )}
            </tbody>
